  ...
  return (
    < LinearGradient
        ...
        <View style={balanceDisplay.details}>
            <CustomTitle title={'Balance'} type='TitleSmall' color={balanceDisplay.white}/>
            <View style={balanceDisplay.amount}>
                <Ionicons testID="arrow-up" name={'arrow-up'} size={22} color={colorsTheme.white} />
                <CustomTitle 
                    title={`$ ${validIncome.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`} 
                    type='TitleSmall' 
                    color={balanceDisplay.white}/>
            </View>
        </View>
        <View style={balanceDisplay.details}>
            <CustomTitle 
                title={`$ ${balance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`} 
                type='TitleMedium' 
                color={balanceDisplay.white}/>
            <View style={balanceDisplay.amount}>
                <Ionicons testID="arrow-down" name={'arrow-down'} size={22} color={colorsTheme.white} />
                <CustomTitle 
                    title={`$ ${validExpense.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`} 
                    type='TitleSmall' 
                    color={balanceDisplay.white}/>
            </View>
        </View>
